...
Q.ajax = function(method, url, data, headers) {
    ...
    if (headers) {for(const name in headers) {options.headers[name] = headers[name];}}
    ...
    if (elem) {
        ...
        if (files && files[0]) {
            reader.onload = function (event) {
                const b64 = btoa(event.target.result);
...
Q.score_input = function(elem, reference) {
    ...
    elem.onkeyup = elem.onchange = function(evt) {
      const score = Q.score_password(elem.value.trim());
      const r = Math.round(255*Math.max(0,Math.min(2-2*score/reference,1)));
      const g = Math.round(255*Math.max(0,Math.min(2*score/reference,1)));
...
Q.trap_form = function (action, elem_id) {
    Q('#' + elem_id + ' form:not(.no-form-trap)').forEach(function (form) {
        ...
        const clickable = 'input[type=submit], input[type=image], button[type=submit], button:not([type])';        
        form.querySelectorAll(clickable).forEach(function (elem) {
            elem.onclick = function(event) {
                ...
                const form_data = new FormData(form); // Allows file uploads.
...
Q.handle_flash = function() {
    ...
    const make_handler = function(elem) {
        return function (event) { 
