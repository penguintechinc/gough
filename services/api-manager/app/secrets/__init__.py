...
async def get_secrets_manager(backend: str | None = None) -> BaseSecretsManager:
...
def get_backend_registry() -> dict[str, type[BaseSecretsManager]]:
    """Get the registry of available backends.

    Ensures all backends are registered before returning.
...
# Ensure backends are registered on module import
_ensure_backends_registered()
