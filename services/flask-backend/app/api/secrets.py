...
def get_secret(secret_path: str):
    ...
    try:
        ...
        secret_data = manager.get_secret(secret_path)
...
@auth_required()
@roles_required("admin")
def set_secret(secret_path: str):
    ...
    try:
        ...
        # Store the secret
        manager.set_secret(secret_path, secret_data)
...
@auth_required()
@roles_required("admin")
def delete_secret(secret_path: str):
    """Delete a secret.
    ...
    try:
        manager = get_secrets_manager()
        deleted = manager.delete_secret(secret_path)
...
def list_secrets():
    ...
    try:
        manager = get_secrets_manager()
        secrets = manager.list_secrets(path)
