...
def require_team_permission(required_role: str = "member") -> Callable:
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        def wrapped(*args, **kwargs):
            ...
            if not check_team_access(current_user.id, team_id, required_role):
                return jsonify({"error": "Forbidden"}), 403
...
def require_resource_permission(permission: str = "read") -> Callable:
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        def wrapped(*args, **kwargs):
            ...
            if not resource_type or not resource_id or not current_user:
                return jsonify({"error": "Forbidden"}), 403

            if not check_resource_permission(
                current_user.id, resource_type, resource_id, permission
            ):
                return jsonify({"error": "Forbidden"}), 403
